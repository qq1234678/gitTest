 ...
 * 获取到的打印机设备的类型。
 */
export declare enum LPA_DeviceType {
    /** 本地打印机机 */
    Local = 1,
    /** 局域网内其他电脑上的打印机 */
    Net = 2,
...
export interface LPA_JobOptions {
 ...
 * 打印任务参数。
 */
export interface LPA_JobPrintOptions extends LPA_JobOptions {
    /** 打印任务名称 */
    jobName?: string;
...
export interface LPA_JobPreviewOptions extends LPA_JobOptions {
    /** 预览背景模式 */
 ...
 * 绘制圆形相关选项。
 */
export interface DrawCircleOptions extends FillCircleOptions {
    /** 圆边框线条宽度，单位毫米 */
    lineWidth?: number;
    /** 是否绘制填充圆 */
    fill?: boolean;
 ...
 */
export interface DrawImageDataOptions extends DrawBaseOptions {
    /** 图片base64数据 */
    data?: string;
    ...
    drawHeight?: number;
    /** 黑白打印的灰度阀值，默认为192；0 表示使用参数设置中的值；256 表示取消黑白打印，用灰度打印；257 表示直接打印图片原来的颜色 */
    threshold?: number;
    /** 图片格式 */
    format?: number;
    /** data中图片的实际宽度 */
...
declare type DrawType = 'text' | 'barcode' | 'qrcode' | 'pdf417' | 'dataMatrix' | 'image' | 'imageD' | 'rectangle' | 'roundRectangle' | 'ellipse' | 'line';
...
declare type DrawItemOptions = TextItemOptions | BarcodeItemOptions | QRCodeItemOptions | PDF417ItemOptions | ImageDItemOptions | ImageDItemOptions | RectItemOptions | CircleItemOptions | LineItemOptions;
/**
...
export declare type RequestHeaders = Record<string, string | number | boolean>;
export declare type Method = 'get' | 'GET' | 'post' | 'POST';
...
export declare const utils: {
    ...
    parseMargin(options: LPA_JobPrintOptions): number[];
    ...
    request(config: RequestConfig, resolve: (result: LPA_Response) => void): void;
    requestXMLHttp(config: RequestConfig, resolve: (result: LPA_Response) => void): void;
    requestNodeHttp(config: RequestConfig, resolve: (result: LPA_Response) => void): void;
    /**
    ...
    getTimeout(timeout?: number, type?: LPA_DeviceType): number;
    getAgent(): string;
    isWin32(): number;
    isWin64(): number;
    isWindows(): boolean;
