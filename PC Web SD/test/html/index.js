...
async function drawTextTest() {
	...
	var m = '\n\n';
	var space=" ";
    var text = m+space.repeat(24)+'男'+'\n'+space.repeat(24)+'27'+'\n\n'+space.repeat(13)+'罗仁涛'+ '\n'+space.repeat(9)+'25血液流变学检查';
	//@上海道臻信息技术有限公司#
    // var text = "www.dothatnech.com";
        
    if (await api.startJob({ width: width, height: height, orientation: orientation, jobName: jobName })) {
        //文本
        if(api.drawText({ text: text, width: width, height: height, fontHeight: fontHeight })){
			// 打开打印机
			if (isPrintJob() && !(await openPrinter())) return;
			...
			await api.draw1DBarcode({
				text: text,  //text是要在条形码上编码的文本信息
				name:name,
			    x: 10,  //x和y是条形码的左上角起点坐标
			    y: margin,
			    width: width - margin * 2, //条形码所占的宽度 30
			    height: height - margin * 2, //10
			    textHeight:3,  //文本在条形码上的高度
				textWidth:1,
			});
...
async function drawBarcodeTest() {
    ...
    var margin = 5;
                                 //打印，width和height是纸张的宽度和高度
    if (await api.startJob({width: width, height: height, orientation: orientation, jobName: jobName})) {
			await api.draw1DBarcode({
				text: text,  //text是要在条形码上编码的文本信息
				
			    x: margin,  //x和y是条形码的左上角起点坐标
			    y: margin,
			    width: width - margin * 2, //条形码所占的宽度 30
			    height: height - margin * 2, //10
			    textHeight:5,  //文本在条形码上的高度
			});
        //提交打印任务
        await api.commitJob();
...
function addPreview(data) {
    ...
    var img = document.createElement('img');//用于创建新的HTML元素节点
    img.src = data;
    previewGroup.appendChild(img);//新的子节点添加到现有的父节点中
    // 换行
