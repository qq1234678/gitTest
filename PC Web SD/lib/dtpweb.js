(function (global, factory) {
    ...
    class DTPWeb {
        ...
        async drawText(options) {
            ...
            // 绘制区域 regionCorners显示区域四个角的删除矩形
        ...
        async draw1DBarcode(options) {
			//将传入的options对象中的text属性取出，并将其赋值给options变量
            options = utils.getRequestData(['text'], [options]);
            if (!options.text)
                return false;
            //将传入的options对象中涉及到单位转换的属性进行转换，并传入当前标签的页边距（this._margins）作为参数。
            options = utils.unitConvertOfDrawBase(options, this._margins);
			//函数将文本高度和文本与条码之间的间距进行单位转换
			// options.textHeight 一维码中供人识读文本的高度  
            options.textHeight = utils.unitConvert(options.textHeight);
			console.log(options.textHeight+"===========3");//400px
			//options.textBarSpace一维码供人识读文本和条码的垂直间距
            options.textBarSpace = utils.unitConvert(options.textBarSpace);
			//options.fontName 一维码中供人识读文本的字体名称
            options.fontName = this.getFontName(options.fontName);
			console.log(options.fontName+"===========5");
			options.textWidth=utils.unitConvert(options.textWidth);
			console.log(options.textWidth+"===========5");
			
			

            //使用参数ACTIONS.Draw1DBarcode和options调用标签打印机的API，并等待API返回结果。如果API返回的结果状态码等于LPA_Result.OK，则函数返回true，否则返回false。
